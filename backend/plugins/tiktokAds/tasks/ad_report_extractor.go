...
func ExtractAdReport(taskCtx plugin.SubTaskContext) errors.Error {
	...
	extractor, err := helper.NewApiExtractor(helper.ApiExtractorArgs{
		...
		Extract: func(resData *helper.RawData) ([]interface{}, errors.Error) {
			...
			res := &models.AdReportListItem{}
			err := json.Unmarshal(resData.Data, res)
			if err != nil {
				return nil, errors.Convert(err)
			}
			report := res.Metrics
			report.AdId = res.Dimensions.AdID
...
var ExtractAdReportMeta = plugin.SubTaskMeta{
	Name:             "ExtractAdReport",
	EntryPoint:       ExtractAdReport,
